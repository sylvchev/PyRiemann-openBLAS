...
for i in range(0, len(size)):
    ...
    t = timeit.Timer("mean_logdet(covmats.numpy_array)",
                     setup="from __main__ import covmats; from oldPyRiemann.mean import mean_logdet; import Utils.OpenBLAS")
    old_time = t.timeit(number=size[len(size) - i - 1]) / size[len(size) - i - 1]

    t = timeit.Timer("covmats.reset_matrices_fields(); Mean.log_determinant(covmats)",
