...
for i in range(0, len(size)):
    ...
    t = timeit.Timer("distance_kullback(covmat1.numpy_array, covmat2.numpy_array)",
                     setup="from __main__ import covmat1; from __main__ import covmat2; from oldPyRiemann.distance import distance_kullback; import Utils.OpenBLAS")
    old_time = t.timeit(number=size[len(size) - i - 1]) / size[len(size) - i - 1]

    t = timeit.Timer("covmat1.reset_fields(); covmat2.reset_fields(); Distance.kullback(covmat1, covmat2)",
