...
for i in range(0, len(size)):
    ...
    t = timeit.Timer("geodesic_euclid(covmat1.numpy_array, covmat2.numpy_array)",
                     setup="from __main__ import covmat1; from __main__ import covmat2; from oldPyRiemann.geodesic import geodesic_euclid; import Utils.OpenBLAS")
    old_time = t.timeit(number=size[len(size) - i - 1]) / size[len(size) - i - 1]

    t = timeit.Timer("covmat1.reset_fields(); covmat2.reset_fields(); Geodesic.euclidean(covmat1, covmat2)",
