...
int main()
{
	...
	for (unsigned int i = 0; i < 9; i++)
	{
		...
		{			
			if(system("clear"));
			cout << completionText;
			cout << "Benchmarking euclidean distance : " << ((double)(i*NBREPET + j)/(double)(9*NBREPET))*100 << "%" << endl;
			...
			timer.tic();
			Distance::EuclideanDistance(covMats1[i], covMats2[i]);
			time[i] += timer.toc();
		}
	}
	completionText += "Benchmarking euclidean distance : 100%\n\n";
