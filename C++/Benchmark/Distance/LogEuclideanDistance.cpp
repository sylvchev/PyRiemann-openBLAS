...
int main()
{
	...
	for (unsigned int i = 0; i < 9; i++)
	{
		...
		{			
			if(system("clear"));
			cout << completionText;
			cout << "Benchmarking log euclidean distance : " << ((double)(i*NBREPET + j)/(double)(9*NBREPET))*100 << "%" << endl;
			...
			double start = clock();			
			Distance::LogEuclideanDistance(covMats1[i], covMats2[i]);
			time[i] += clock() - start;
		}
	}
	completionText += "Benchmarking log euclidean distance : 100%\n\n";
