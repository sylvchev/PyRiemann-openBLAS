...
def tangent_space(covmats, Cref):
    ...
    coeffs = (numpy.sqrt(2) * numpy.triu(numpy.ones((Ne,Ne)),1) + numpy.eye(Ne))[idx]
...
def untangent_space(T, Cref):
    ...
    for i in range(Nt):
        covmats[i] = numpy.diag(numpy.diag(covmats[i])) + numpy.triu(covmats[i], 1) / numpy.sqrt(2) + numpy.triu(covmats[i], 1).T / numpy.sqrt(2)
